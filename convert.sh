...
wav_to_txt() {
  ...
  ffmpeg -y \
    ...
    -map_metadata -1   `# ????` \
    out-$hash.raw            `# output to out.raw`

  sbcinfo out-$hash.raw
  xxd -i out-$hash.raw            `# output in C include file style` \
